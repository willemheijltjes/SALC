...
\]
The primary aims of these approaches is to eliminate $\alpha$-conversion and to streamline substitution. Consequently, while they can \emph{identify} the spine, they do not readily isolate it for duplication, as required for spinal full laziness.
...
{ \small
\drv{X ; | ; Z} {:}{:}{=} $a$ $\vert$ \drv{Y \rightarrow \drv[yellow]{X_{2} ; | ; Z_{2}}} $\vert$ \drv{\drv[yellow]{X_{1} ; | ; Z_{1}} \wedge \drv[yellow]{X_{2} ; | ; Z_{2}}} $\vert$ \drv{\drv[yellow]{X ; | ; Y_{1}} ; -[r] ; \drv[yellow]{Y_{2} ; | ; Z}}
}
	\caption{Derivations}
	\label{fig:derivations}
	\end{subfigure}
	...
	\centering
	\scalebox{0.9}{ \drv{\top ; -[a] ; X \rightarrow X}} \hspace{0.1cm}
		\scalebox{0.9}{ \drv{(X \rightarrow Y) \wedge X ; -[\apprule] ; Y}} \hspace{0.1cm}
		\scalebox{0.9}{ \drv{X ; -[\sharerule] ; X \wedge \dots \wedge X}} \\[0.2cm]
		\scalebox{0.9}{ \drv{(X \rightarrow Y) \wedge Z ; -[s] ; X \rightarrow (Y \wedge Z)}} \hspace{0.05cm}
	\scalebox{0.9}{ \drv{X ; -[\lambda] ; Y \rightarrow (X \wedge Y)}} {:}{=} \scalebox{0.9}{ \drv{\drv[yellow]{\top ; -[a] ; Y \rightarrow Y} \wedge X ; -[s] ; Y \rightarrow (X \wedge Y)}}
	\caption{Axiom, Application, Contraction, Switch, and Abstraction}
