...
\[
	{\small   \begin{tikzpicture}[auto, scale = 0.75]
		...
		%\filldraw[fill = yellow!20, draw = yellow!70, rounded corners] g(0.45, -2.75) rectangle (1.75, -4);
		%%%%%%%%%%%%%
...
\footnote{Interestingly, Balabonski \cite{} shows that for \emph{weak} reduction (where one does not reduce under an abstraction) full laziness, spinal full laziness, and optimal reduction all require a comparable number of beta-steps to reach a normal form.}
\end{description}
...
\[
	\red\lambda.(\lambda.({\black((\lambda.\var)\var)^\dirR}\var)^{\dirR\black\dirL})^\dirR
\]
The primary aim of these ideas is to streamline substitutions, avoiding their unnecessary duplication. Consequently, while they \emph{identify} the spine, they do not readily isolate it for duplication. 


Blanc, L\'evy and Maranget 

\cite{blanc2007sharing} use a labelled calculus.
